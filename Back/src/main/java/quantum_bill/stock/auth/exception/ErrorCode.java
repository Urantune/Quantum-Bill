package quantum_bill.stock.auth.exception;

import lombok.Getter;
import org.springframework.http.HttpStatus;
import org.springframework.http.HttpStatusCode;

@Getter
public enum ErrorCode {
    UNCATEGORIZED_EXCEPTION(9999, "Uncategorized Exception", HttpStatus.INTERNAL_SERVER_ERROR),
    INVALID_KEY(1001, "INVALID KEY", HttpStatus.BAD_REQUEST),
    INVALID_TOKEN(1001, "INVALID TOKEN", HttpStatus.BAD_REQUEST),
    USER_EXISTED(1002, "User existed", HttpStatus.BAD_REQUEST),
    USERNAME_INVALID(1003, "Username must be {min} - 15 character", HttpStatus.BAD_REQUEST),
    INVALID_PASSWORD(1004, "Password must be {min} - 15 character", HttpStatus.BAD_REQUEST),
    USER_NOT_EXISTED(1005, "User not existed", HttpStatus.NOT_FOUND),
    UNAUTHENTICATED(1006, "Unauthenticated", HttpStatus.UNAUTHORIZED),
    ROLE_NOT_EXISTED(1007, "ROLE NOT EXISTED", HttpStatus.FORBIDDEN),
    EMAIL_EXISTED(1009, "Email existed", HttpStatus.BAD_REQUEST),
    USER_NOT_ACTIVE(1010, "Account not verify", HttpStatus.BAD_REQUEST),
    TOO_MANY_REQUESTS(1011, "Please wait until the waiting time is over (60s).", HttpStatus.BAD_REQUEST),
    PASSWORD_NOT_MATCH(1012, "Passwords don't match", HttpStatus.BAD_REQUEST),
    PASSWORD_DUPLICATE(1013, "New password must be different from old password", HttpStatus.BAD_REQUEST),
    ARTIST_EXISTED(1014, "Artist existed", HttpStatus.BAD_REQUEST),
    ARTIST_NOT_FOUND(1015, "Artist not existed", HttpStatus.BAD_REQUEST),
    GENRE_EXISTED(1016, "Genre existed", HttpStatus.BAD_REQUEST),
    GENRE_NOT_FOUND(1015, "Genre not found", HttpStatus.BAD_REQUEST),
    ALBUM_NOT_FOUND(1015, "Album not found", HttpStatus.BAD_REQUEST),
    LOGIN_ERROR(1018, "Login Error", HttpStatus.UNAUTHORIZED),
    FORGOT_PASSWORD_EMAIL(1019, "Email not found", HttpStatus.BAD_REQUEST),
    UNAUTHORIZED(1007, "You do not have permission", HttpStatus.FORBIDDEN);


    ErrorCode(int code, String message, HttpStatusCode statusCode) {
        this.code = code;
        this.message = message;
        this.statusCode = statusCode;
    }

    private int code;
    private String message;
    private HttpStatusCode statusCode;
}
