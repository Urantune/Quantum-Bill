...
import { PlusCircle, Edit3, Building, TrendingUp, AlertTriangle, Search, Filter, ChevronLeft, ChevronRight, Trash2, Gauge } from 'lucide-react';
...
const OwnerDashboard = () => {
    ...
    const fetchOwnerData = async (page = 0) => {
        ...
        try {
            const [allItems, txItems] = await Promise.all([
                investorService.getMyStocks(),
                investorService.getCompanyTransactions().catch(() => []),
            ]);
            const pendingItems = await investorService.getPendingOrders().catch(() => []);
    ...
    const handleDelete = async (stock) => {
        ...
        await investorService.deleteStock(stock.id);
        fetchOwnerData(currentPage);
    };

    const handleApproveOrder = async (orderId) => {
        await investorService.approveOrder(orderId);
        fetchOwnerData(currentPage);
    };

    const handleRejectOrder = async (orderId) => {
        await investorService.rejectOrder(orderId);
        fetchOwnerData(currentPage);
    };
        ...
        <motion.div {...MOTION.pageTransition} className="space-y-6">
            ...
            <div className="panel overflow-hidden">
                <div className="px-6 py-4 border-b border-border-subtle flex justify-between items-center">
                    <h2 className="text-sm font-semibold text-text-primary">Danh mục chứng khoán phát hành</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tr className="border-b border-border-subtle bg-white/[0.02] text-xs font-semibold text-text-secondary uppercase tracking-wider">
                            ...
                            <th className="px-6 py-3.5 text-center">Điều chỉnh giá (±9%)</th>
                        ...
                        {stocks.length === 0 ? (
                            <tr>
                                <td colSpan="7" className="text-center py-10 text-text-muted text-xs">Không tìm thấy mã niêm yết nào khớp với bộ lọc.</td>
                            </tr>
                        ) : (
                            stocks.map((stock) => (
                                <tr key={stock.id} className="hover:bg-white/[0.01] transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <button
                                            onClick={() => setAdjustModal({ open: true, stock })}
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <Link
                                            to={`/investor/register?edit=${stock.id}`}
                                            className="p-1.5 rounded-md hover:bg-white/5 text-text-secondary hover:text-primary transition-colors inline-block"
                                            title="Sửa thông tin"
                                        >
            ...
            <div className="panel overflow-hidden">
                <div className="px-6 py-4 border-b border-border-subtle">
                    <h2 className="text-sm font-semibold text-text-primary">Giao dịch gần đây của doanh nghiệp</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead>
            ...
            <PriceAdjustModal
                ...
                onSuccess={() => fetchOwnerData(currentPage)}
            />
        </motion.div>
    );
};
