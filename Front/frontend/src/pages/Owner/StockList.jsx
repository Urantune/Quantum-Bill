...
import StockChart from "@/components/Owner/StockChart.jsx";
...
export default function StockList() {
    ...
    const [holdings, setHoldings] = useState({}); // stockId → quantity
    ...
    useEffect(() => {
        ...
        ownerService.getPortfolio()
            .then(res => {
                const map = {};
                (res.data?.holdings || []).forEach(h => { map[h.stockId] = h.quantity; });
                setHoldings(map);
            })
            .catch(() => {});
    ...
    useEffect(() => {
        ...
        ownerService.getStockHistory(selectedStock.id)
    ...
    const placeOrder = async (type) => {
        ...
        try {
            ...
            const response = type === "BUY"
                ? await ownerService.buyStock(payload)
                : await ownerService.sellStock(payload);
                ...
                <div className="space-y-5">
                    ...
                    <div className="panel p-5 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                        <div className="md:col-span-2">
                            ...
                            {selectedStock && holdings[selectedStock.id] > 0 && (
                                <div className="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-xs text-primary font-semibold">
                                    📦 Đang sở hữu: {holdings[selectedStock.id].toLocaleString()} cổ phiếu
                                </div>
                            )}
                        </div>
                        <div>
                                ...
                                >
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <div className="text-right">
                                            ...
                                            {owned > 0 && (
                                                <div className="text-xs text-primary mt-0.5">Sở hữu: {owned.toLocaleString()} cp</div>
                                            )}
